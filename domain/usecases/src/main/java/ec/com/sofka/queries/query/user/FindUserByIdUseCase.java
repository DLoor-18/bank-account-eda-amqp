...
public class FindUserByIdUseCase implements IUseCaseGetElement<GetElementQuery, UserResponse> {
    ...
    public Mono<User> getUserByAggregate(String aggregateId) {
        return eventStore.findAggregate(aggregateId)
                ...
                .collectList()
                .map(eventsList -> AccountAggregate.from(aggregateId, eventsList).getUser());
    }
