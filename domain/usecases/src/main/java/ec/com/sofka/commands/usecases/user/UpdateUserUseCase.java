    ...
    public Mono<UserResponse> execute(UserCommand userCommand) {
        return userRepository.findByEmail(userCommand.getEmail())
                ...
                .switchIfEmpty(Mono.defer(() -> {
                    return repository.findAggregate(userCommand.getAggregateId())
                            .collectList()
                            .map(domainEvent -> AuthAggregate.from(userCommand.getAggregateId(), domainEvent))
                            .flatMap(authAggregate -> {
